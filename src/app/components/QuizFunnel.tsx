...
import { COPY as DEFAULT_COPY } from "@/funnels/seed-mexico-qz/copy";
import { useAnalytics } from "@/hooks/useAnalytics";

type CopyType = typeof DEFAULT_COPY;
...
export default function QuizFunnel({
  ...
  function goToStep(n: number) {
    ...
    if (!el) { setStep(n); trackStep(n); setSelectedInStep(null); return; }
  ...
  function handleCta(nextStep = step + 1) {
    desdeSlide.current = step;
    trackCta(step);
    goToStep(nextStep);
  }

  function handleAnswer(stepIdx: number, answerText: string) {
    if (selectedInStep !== null || isAnimating) return;
    const answerIdx = COPY[`step${stepIdx}` as keyof CopyType] &&
      "answers" in (COPY[`step${stepIdx}` as keyof CopyType] as object)
      ? (COPY[`step${stepIdx}` as keyof CopyType] as { answers: { text: string }[] }).answers.findIndex(a => a.text === answerText)
      : 0;
    setSelectedInStep(answerIdx);
    setQuizAnswers((prev) => ({ ...prev, [stepIdx]: answerText }));
    trackCta(stepIdx);
    setTimeout(() => goToStep(stepIdx + 1), 400);
  }
  ...
  const isQuizStep = QUIZ_STEPS.includes(step);
        ...
        <div className="h-full overflow-y-auto relative z-10">
          <div className="px-14 md:px-24 pt-3 pb-1 md:pt-8" style={{ maxWidth:"900px", margin:"0 auto" }}>
            <div ref={contentRef} style={{ willChange:"transform" }}>
                ...
                <div>
                  ...
                  <div className="flex justify-center">
                    <Image
                      ...
                      className="w-full max-h-[30vh] md:max-h-[38vh]"
              ...
              {step === 1 && (
                <div>
                  <p style={{ color:"#14C9B8", fontSize:"0.8125rem", fontWeight:700, textTransform:"uppercase",
                ...
                <div>
                  ...
                  {(quizAnswers[1] || quizAnswers[2]) && (
                    <div className="rounded-xl border mb-4"
                      ...
                      <p style={{ color:"#14C9B8", fontSize:"0.8rem", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:"8px" }}>
                        Tu perfil
                      </p>
                      {quizAnswers[1] && (
                  ...
                  <form id="quiz-form" onSubmit={handleSubmit} className="flex flex-col gap-3">
                    <div>
                      ...
                      <input type="text" required placeholder={COPY.step5.form.namePlaceholder}
                        value={form.nombre} onChange={(e) => setForm((p) => ({ ...p, nombre:e.target.value }))}
                        className="w-full rounded-xl transition-all duration-200 focus:outline-none"
                        style={{ background:"#0d1117", border:"1px solid #1e2535", color:"#fff", padding:"14px 16px", fontSize:"16px" }}
                        onFocus={(e) => (e.currentTarget.style.borderColor = "#14C9B8")}
                        onBlur={(e)  => (e.currentTarget.style.borderColor = "#1e2535")}/>
                    </div>
                    <div>
          ...
          <div className="px-14 md:px-24" style={{ maxWidth:"900px", margin:"0 auto" }}>
            {step === 5 && (
              <button type="submit" form="quiz-form" disabled={submitting}
                className="w-full rounded-xl active:scale-[0.98]"
                style={{ background: submitting ? "#0a8a80" : "#14C9B8", color:"#fff", padding:"17px 24px",
                  fontSize:"1.1rem", fontFamily:"var(--font-poppins)", fontWeight:900, letterSpacing:"0.05em",
                  lineHeight:1.2, cursor: submitting ? "not-allowed" : "pointer", border:"none" }}>
                {submitting ? COPY.step5.ctaLoading : COPY.step5.cta}
              </button>
            )}
